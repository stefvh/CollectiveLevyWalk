...
if [ "$BUILD" == "true" ]; then 
    ...
    parallel --nonall -S jnauta@node0,jnauta@node1,jnauta@node2,jnauta@node3,jnauta@node4 'rm -rf CollectiveLevyWalk; tar -xzf CollectiveLevyWalk.tar.gz; cd CollectiveLevyWalk; if [[ ! -e build ]]; then mkdir build; fi; cd build; cmake -DCMAKE_BUILD_TYPE=Release ..; make'
fi
